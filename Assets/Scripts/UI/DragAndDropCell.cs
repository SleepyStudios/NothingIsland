...
public class DragAndDropCell : MonoBehaviour, IDropHandler
{
	...
	public void SwapItems(DragAndDropCell firstCell, DragAndDropCell secondCell)
	{
		if ((firstCell != null) && (secondCell != null))
		{
			...
			{
                // Crafting
                Item item = firstItem.transform.parent.GetComponent<ItemSlot>().GetItem();
                if (item.recipes.Length > 0 && secondItem != null) {
                    foreach (Recipe recipe in item.recipes) {
                        if (recipe.other.name == secondItem.transform.parent.GetComponent<ItemSlot>().GetItem().name) {
                            DestroyImmediate(firstItem.gameObject);
                            DestroyImmediate(secondItem.gameObject);

                            FindObjectOfType<Tooltip>().Reset();
                            FindObjectOfType<InteractMenu>().Cancel();

                            Inventory inv = FindObjectOfType<Inventory>();
                            foreach(RecipeResult result in recipe.results) {
                                for (int i = 0; i < result.num; i++) {
                                    inv.PickupItem(result.item);
                                }
                            }
